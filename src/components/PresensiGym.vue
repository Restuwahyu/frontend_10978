...
export default {
  ...
  data() {
    return {
      headers: [
        { text: "ID Presensi Gym", value: "id_booking_gym" },
        { text: "Nama Member", value: "nama_member" },
        { text: "Jam Mulai", value: "waktu_mulai" },
        { text: "Jam Selesai", value: "waktu_selesai" },
  ...
  methods: {
    ...
    Update() {
      var url = this.$api + "/booking_gyms/" + this.editId;
      this.load = true;
    ...
    EditHandler(item) {
      this.inputType = "Ubah";
      this.editId = item.id_booking_gym;
    ...
    downloadPDF(
      ...
      doc.text(" ", 20, 3);
      doc.setFont("helvetica", "bold");
      doc.text("STRUK PRESENSI GYM", 1, 4);
      doc.setFont("helvetica", "normal");
      doc.text("No Struk        : " + $no_struk, 1, 5);
      doc.text("Tanggal         : " + $tanggal_presensi, 1, 6);
      doc.text(" ", 20, 7);
      doc.text("Member         : " + $id_member + " / " + $nama_member, 1, 8);
      doc.text(
        "Slot Waktu     : " + $waktu_mulai + " - " + $waktu_selesai,
        1,
        9
      );
      doc.save("Struk Presensi Gym -" + $id_member + ".pdf");
    },
  },
  computed: {
    formTitle() {
...
<template>
  <v-main class="PG">
    <h3 class="text font-weight-medium mb-5">Data Presensi Gym</h3>
    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="presensi_gyms" :search="search">
        <template v-slot:item.actions="{ item }">
          <v-btn
            v-if="item.status == 'Telah Dipresensi'"
            icon
            small
