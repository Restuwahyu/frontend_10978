...
export default {
  ...
  methods: {
    ...
    ReadData() {
      let url = this.$api + "/izin_instrukturs";
      this.$http
        .get(url, {
          headers: {
    ...
    Update() {
      var url = this.$api + "/izin_instruktur/" + this.editId;
      this.load = true;
    ...
    EditHandler(item) {
      this.inputType = "Ubah";
      this.editId = item.id_izin_instruktur;
  ...
  computed: {
    ...
    getNamaInstrukturPengganti() {
      return (id_instruktur_pengganti) => {
        const instruktur = this.instrukturList.find(
    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:item.actions="{ item }"
          ...
          >
            <v-icon color="yellow">mdi-application-edit</v-icon>
          </v-btn>
        </template>
